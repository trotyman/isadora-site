...
// Logo SVG inline (para funcionar em ambiente serverless)
...
const WATERMARK_CONFIG = {
  // Tamanho da marca d'água como porcentagem da largura da imagem
  widthPercent: 20,
  // Opacidade (0-1) - mais visível
  opacity: 0.7,
  // Margem do canto em pixels
  margin: 30,
  // Posição: 'bottom-right', 'bottom-left', 'top-right', 'top-left', 'center'
 ...
 * @returns {Promise<Buffer>} - Buffer da imagem com marca d'água
 */
async function applyWatermark(imageBuffer, mimeType) {
  try {
    // Verificar se é uma imagem (não aplicar em PDFs, etc)
    if (!mimeType || !mimeType.startsWith('image/')) {
      console.log('Tipo não é imagem, pulando watermark:', mimeType);
      return imageBuffer;
    }
    ...
    console.log('Aplicando watermark em imagem:', metadata.width, 'x', metadata.height);

    // Calcular tamanho da marca d'água
    const watermarkWidth = Math.round(metadata.width * (WATERMARK_CONFIG.widthPercent / 100));
    
    // Criar a marca d'água a partir do SVG inline
    let watermarkBuffer;
    try {
      watermarkBuffer = await sharp(Buffer.from(WATERMARK_SVG))
        .resize(watermarkWidth)
        .png()
        .toBuffer();
    } catch (logoError) {
      console.error('Erro ao processar logo SVG:', logoError);
      return imageBuffer;
    }

    // Obter dimensões da marca d'água redimensionada
    ...
    if (mimeType === 'image/jpeg') {
      ...
      return result.webp({ quality: 90 }).toBuffer();
    } else if (mimeType === 'image/gif') {
      // GIF não suporta composição complexa no sharp, retornar como PNG
      return result.png().toBuffer();
    }
 ...
 */
function calculatePosition(imgWidth, imgHeight, wmWidth, wmHeight, position, margin) {
  let left, top;
